...
public class IpProxy {
    public static ResultBody proxy(String province, String city, int count) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            try {
                response = HttpUtil.createGet(url).form(formMap).execute();
                if (!response.isOk()) {
                ...
                log.error("[IPProxy] {}, retry...", exception.getMessage(), exception);
                finalException = new RuntimeException(exception.getMessage(), exception);
                try {
                    Thread.sleep(500);
