            ...
            synchronized (UserSetting.class) {
                if (userSetting == null) {
                    ...
                    if (file.exists()) {
                        ...
                        String oldVersion = Optional.ofNullable(JSON.parseObject(read).get("version")).map(Object::toString).orElse(null);
                    ...
                    if (!file.exists()) {
                        ...
                        // 获取WebDriver默认设置
                        Assert.isTrue(new File(setting.getDriver()).exists(), () -> new RuntimeException("Driver file not found: " + setting.getDriver()));
                        Assert.isTrue(new File(setting.getBinary()).exists(), () -> new RuntimeException("Binary file not found: " + setting.getBinary()));
