...
import com.cxxwl96.autoanswer.model.*;
import com.cxxwl96.autoanswer.service.SurveyService;
import com.cxxwl96.autoanswer.utils.*;
...
public class SurveyServiceImpl implements SurveyService {
    ...
    public void submit(String domain, String linkId, SubmitInfo submitInfo) {
        ...
        if (submitInfo.getProxyType() == ProxyType.ASSIGN_PROXY) {
            Assert.isTrue(StrUtil.isNotBlank(submitInfo.getProvince()) && StrUtil.isNotBlank(submitInfo.getCity()),
                    "请选择代理城市");
        }
        ...
        for (int i = 0; i < surveyAnswers.size(); i++) {
            for (int j = 0; j < surveyAnswers.get(i).size(); j++) {
                ...
                if (CollUtil.isEmpty(answers)) {
                    // 获取选择率降序的选项
                    List<String> tabs = sortOptionTabs(subject);
    ...
    private List<String> sortOptionTabs(Subject subject) {
        ...
        for (Option option : subject.getOptions()) {
            ...
            int selectance = option.getRules().stream().map(Rule::getSelectance).max(Comparator.comparing(a -> a)).orElse(NO_SELECTANCE);
            selectance = Math.max(selectance, option.getDefaultSelectance());

            map.put(option.getTab(), selectance);
        }
        if (allEquals(new ArrayList<>(map.values()))) {
            List<String> tabs = new ArrayList<>(map.keySet());
            shuffleList(tabs);
            return tabs;
        } else {
            return map.entrySet()
